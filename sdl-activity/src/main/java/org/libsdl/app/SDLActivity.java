...
*/
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mBrokenLibraries) {
            ...
            dlgAlert.setPositiveButton("Exit" , (dialog , id) -> {
        ...
        if (intent != null && intent.getData() != null) {
            var filename = intent.getData().getPath();
            if (filename != null) {
    ...
    public static int getCurrentOrientation() {
        ...
        switch (display.getRotation()) {
            case Surface.ROTATION_0 -> result = SDL_ORIENTATION_PORTRAIT;
            case Surface.ROTATION_90 -> result = SDL_ORIENTATION_LANDSCAPE;
            case Surface.ROTATION_180 -> result = SDL_ORIENTATION_PORTRAIT_FLIPPED;
            case Surface.ROTATION_270 -> result = SDL_ORIENTATION_LANDSCAPE_FLIPPED;
        }
    ...
    public void onBackPressed() {
        ...
        //
        var trapBack = SDLActivity.nativeGetHintBoolean("SDL_ANDROID_TRAP_BACK_BUTTON" , false);
        if (trapBack) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.arg1) {
                ...
                case COMMAND_CHANGE_WINDOW_STYLE -> {
                    if (context instanceof Activity) {
                        var window = ((Activity) context).getWindow();
                        if (window != null) {
...
*/
    ...
    public static double getDiagonal() {
        ...
        var dWidthInches = metrics.widthPixels / (double) metrics.xdpi;
        var dHeightInches = metrics.heightPixels / (double) metrics.ydpi;
    ...
    public static boolean isDeXMode() {
        ...
        try {
            final var config = getContext().getResources().getConfiguration();
            final var configClass = config.getClass();
    ...
    public static boolean getManifestEnvironmentVariables() {
        try {
            ...
            var applicationInfo = getContext().getPackageManager()
                    .getApplicationInfo(getContext()
            ...
            for (var key : bundle.keySet()) {
                if (key.startsWith(prefix)) {
                    var name = key.substring(trimLength);
                    var value = bundle.get(key).toString();
    ...
    static class ShowTextInputTask implements Runnable {
        ...
        @Override
        public void run() {
            var params = new RelativeLayout.LayoutParams(w , h + HEIGHT_PADDING);
            params.leftMargin = x;
            params.topMargin = y;
            ...
            var imm = (InputMethodManager) SDL.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
    ...
    protected void messageboxCreateAndShow(Bundle args) {
        ...
        dialog.setOnKeyListener((d , keyCode , event) -> {
            var button = mapping.get(keyCode);
            if (button != null) {
    ...
    public void onSystemUiVisibilityChange(int visibility) {
        if (SDLActivity.mFullscreenModeActive && ((visibility & View.SYSTEM_UI_FLAG_FULLSCREEN) == 0
            ...
            var handler = getWindow().getDecorView().getHandler();
            if (handler != null) {
    ...
    public static int createCustomCursor(int[] colors , int width , int height , int hotSpotX , int hotSpotY) {
        var bitmap = Bitmap.createBitmap(colors , width , height , Bitmap.Config.ARGB_8888);
        ++mLastCursorID;
    ...
    public static boolean setSystemCursor(int cursorID) {
        int cursor_type = switch (cursorID) {
            case SDL_SYSTEM_CURSOR_ARROW -> 1000; //PointerIcon.TYPE_ARROW;
            case SDL_SYSTEM_CURSOR_IBEAM -> 1008; //PointerIcon.TYPE_TEXT;
            case SDL_SYSTEM_CURSOR_WAIT , SDL_SYSTEM_CURSOR_WAITARROW ->
                    1004; //PointerIcon.TYPE_WAIT;
            case SDL_SYSTEM_CURSOR_CROSSHAIR -> 1007; //PointerIcon.TYPE_CROSSHAIR;
            case SDL_SYSTEM_CURSOR_SIZENWSE ->
                    1017; //PointerIcon.TYPE_TOP_LEFT_DIAGONAL_DOUBLE_ARROW;
            case SDL_SYSTEM_CURSOR_SIZENESW ->
                    1016; //PointerIcon.TYPE_TOP_RIGHT_DIAGONAL_DOUBLE_ARROW;
            case SDL_SYSTEM_CURSOR_SIZEWE -> 1014; //PointerIcon.TYPE_HORIZONTAL_DOUBLE_ARROW;
            case SDL_SYSTEM_CURSOR_SIZENS -> 1015; //PointerIcon.TYPE_VERTICAL_DOUBLE_ARROW;
            case SDL_SYSTEM_CURSOR_SIZEALL -> 1020; //PointerIcon.TYPE_GRAB;
            case SDL_SYSTEM_CURSOR_NO -> 1012; //PointerIcon.TYPE_NO_DROP;
            case SDL_SYSTEM_CURSOR_HAND -> 1002; //PointerIcon.TYPE_HAND;
            default -> 0; //PointerIcon.TYPE_NULL;
        };
    ...
    public static int openURL(String url) {
        try {
            var openUrlIntent = new Intent(Intent.ACTION_VIEW);
            openUrlIntent.setData(Uri.parse(url));
            openUrlIntent.addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY
                    | Intent.FLAG_ACTIVITY_MULTIPLE_TASK
                    | Intent.FLAG_ACTIVITY_NEW_DOCUMENT);
            mSingleton.startActivity(openUrlIntent);
        } catch (Exception ex) {
            return -1;
        }
        return 0;
...
//        }
        ...
        if ((source & InputDevice.SOURCE_MOUSE) == InputDevice.SOURCE_MOUSE) {
            ...
            if ((keyCode == KeyEvent.KEYCODE_BACK) || (keyCode == KeyEvent.KEYCODE_FORWARD)) {
                switch (event.getAction()) {
                    case KeyEvent.ACTION_DOWN , KeyEvent.ACTION_UP -> {
    ...
    public boolean onCapturedPointerEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_HOVER_MOVE , MotionEvent.ACTION_MOVE -> {
                x = event.getX(0);
                y = event.getY(0);
...
class SDLInputConnection extends BaseInputConnection implements OnKeyCodeHandler {
    ...
    public boolean deleteSurroundingText(int beforeLength , int afterLength) {
        ...
        // Ref: http://stackoverflow.com/questions/14560344/android-backspace-in-webview-baseinputconnection
