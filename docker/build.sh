#!/usr/bin/env bash
# Build the INSTEAD Launcher in Docker without polluting the host.
...
Builds the INSTEAD Launcher in a Docker container: the project tree and a
persistent Gradle cache are mounted into the container, and it runs as the
current user (project files are not created as root).

Options:
  -h, --help      show this help message
  --rebuild       force a rebuild of the "instead-builder" image before running
  --skip-deps     default build without :instead:downloadDependencies
  --force-deps    default build always re-running :instead:downloadDependencies

Commands (everything after the options is executed inside the container,
working directory = the project root):

  (no command)    default build. Native sources (SDL3/INSTEAD/LuaJIT/etc.) are
                  downloaded only when they are missing from
                  instead/src/main/c/; then assembleDebug runs:
                    ./gradlew --no-daemon [":instead:downloadDependencies"] assembleDebug
  ./gradlew <task>
...
# check on the host whether :instead:downloadDependencies is still needed.
native_deps_present() {
    local c="$REPO_ROOT/instead/src/main/c"
    for d in \
        SDL3/SDL3 SDL3_image/SDL3_image SDL3_mixer/SDL3_mixer SDL3_ttf/SDL3_ttf \
        Instead/Instead LuaJIT libiconv/libiconv
    do
        [[ -d "$c/$d" ]] || return 1
    done
    return 0
...
if (( ${#CMD[@]} == 0 )); then
    if (( FORCE_DEPS )); then
        CMD=(./gradlew --no-daemon ":instead:downloadDependencies" assembleDebug)
    elif (( SKIP_DEPS )); then
        CMD=(./gradlew --no-daemon assembleDebug)
    elif native_deps_present; then
        echo "Native dependencies already present, skipping :instead:downloadDependencies" >&2
        CMD=(./gradlew --no-daemon assembleDebug)
    else
        CMD=(./gradlew --no-daemon ":instead:downloadDependencies" assembleDebug)
    fi
